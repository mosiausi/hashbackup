...
def browse_directories(start_path="."):
    """Norton-Commander–style directory browser. Returns selected path."""
    def _browser(stdscr, current_path):
        curses.curs_set(0)
        selected = 0
        while True:
            stdscr.clear()
            stdscr.addstr(0, 0, f"Browsing: {current_path}")
            stdscr.addstr(1, 0, "↑/↓ = move   Enter = open folder   SPACEBAR = select   T = type path")
            ...
            for idx, entry in enumerate(entries):
                if idx == selected:
                    stdscr.addstr(idx+3, 2, "> " + entry, curses.A_REVERSE)
                else:
                    stdscr.addstr(idx+3, 2, "  " + entry)
            ...
            if key == curses.KEY_UP:
                ...
                candidate = entries[selected]
                if candidate in [".", ".."]:
                    current_path = os.path.abspath(os.path.join(current_path, candidate))
                else:
                    current_path = os.path.join(current_path, candidate)
            elif key == 32:  # SPACEBAR selects
                return current_path
            elif key in (ord('t'), ord('T')):
                curses.endwin()
                typed = input("Type full path: ").strip()
                if os.path.isdir(typed):
                    return typed
                else:
                    input("Invalid path. Press Enter to return...")
                    curses.initscr()

    return curses.wrapper(lambda stdscr: _browser(stdscr, start_path))
...
def get_user_input():
    ...
    # Source folder
    print("📁 SOURCE FOLDER")
    print("   Browse (press ENTER) or type path directly")
    print("   Example: /Users/lnayman/Desktop/")

    first = input("   Source path (or press Enter to browse): ").strip()

    # If user typed a valid path → use it
    if first and os.path.isdir(first):
        source = first
    else:
        # Launch Norton-Commander style selector
        print("\nOpening folder browser...\n")
        source = browse_directories(first if first else ".")
    ...
    if dest_choice == "1":
        ...
        print("   Browse (press ENTER) or type path directly")
        first_dest = input("   Destination path (or press Enter to browse): ").strip()
    
        if first_dest and os.path.isdir(first_dest):
            destination = first_dest
        else:
            destination = browse_directories(first_dest if first_dest else ".")
